...
Commands:	Options:
    ...
    filter 	-F -f -L -p -m -s -D -d -R -N -j -t -O --stdin1 --stdin2 --stdout1 --stdout2 --printPositive
...
Usage examples:
    ...
    F64 build -F Org1.25.f64bin -i more.fasta more2.fasta   // -> Org1.25.v1.f64bin
    F64 build -F NegCont.25.f64bin -i NegCont_R1.fastq NegCont_R2.fastq -K 25
    F64 merge -F Org1.25.v1.f64bin NegCont.25.f64bin -o Exp1.25.f64bin -f Exp1.25.f64flat
...
Disk space: 
the index files are stored as binary int collections. Full human index is 27 GB, SILVA is 3.7 GB. For repeated routine use, the index files can be saved in flat hash format. These load quicker, but can not be merged or updated. 
